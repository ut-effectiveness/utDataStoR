...
```sql
`r display_sql_with_comments('fte', 'fte_current.sql')`
```
...
```sql
`r display_sql_with_comments('fte', 'fte_census.sql')`
```
...
```sql
`r display_sql_with_comments('fte', 'fte_end_of_term.sql')`
```
...
**End of Term**: USHE End of Term FTE is a calculation the based off of the total number of student credit hours, based on the level of the course at the end of an academic term. End of Term FTE is based on snapshot data, this FTE returns a status FTE and may be modified to meet USHE reporting element criteria.
...
Banner FTE is calculated using the same logic as Current FTE, but it is based on data from the Banner system. This sql provides a live count of FTE for a term, which can fluctuate daily and may be adjusted post-semester for changes like withdrawals. For this sql to align with the Current FTE sql, the sql must be run off of the REPT instance of Banner.

```sql
`r display_sql_with_comments('fte', 'banner_fte.sql')`
```

...
## Gender
Gender is the biological sex on a student's birth certificate. The options are M (Male) or F (Female).
...
## Program
Students declare a program of study at point of application. Programs are a prescribed set of courses leading to a degree or certification if the student is degree-seeking, if the student is not degree seeking the program indicates the type of non-degree program they are in.
...
## Student Level
A student's level is the level of degree a student is pursing. The options are: Undergraduate or Graduate.
...
## High School Student
A High School Student is a student is who is taking a course at Utah Tech University while still attending high school.
...
-- this should be something similar to student type but it will break down degree-seeking and non-degree seeking sub groups
...
## Online Tuition Student
An Online Tuition Student is a student who is paying online tuition because they are in an online only program (the program can only be completed online). 

## SQL for Online Tution Student

```sql
SELECT DISTINCT tbraccd_pidm,
                tbraccd_term_code,
                CASE
                    WHEN tbraccd_detail_code IN ('1450', '1451', '1452', '1850')
                        OR tbraccd_detail_code BETWEEN '1460' AND '1474' THEN 'TRUE'
                    ELSE 'FALSE'
                    END AS is_online_tuition -- Online Tuition Codes

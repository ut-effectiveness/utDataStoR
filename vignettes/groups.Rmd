...
WHERE a.is_enrolled IS TRUE
  ...
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.gender_code, a.version_desc
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.ipeds_race_ethnicity, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.us_citizenship_desc, a.version_desc
ORDER BY a.term_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND b.primary_visa_type_code = 'F1'
  AND b.us_citizenship_code = '2'
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, residency, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, age_band, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.is_first_generation, a.version_desc
ORDER BY a.term_desc;
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND b.is_veteran IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.is_veteran, a.version_desc
ORDER BY a.term_desc;
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND b.is_athlete IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.is_athlete, a.version_desc
ORDER BY a.term_desc;
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.first_admit_country_desc, a.version_desc
ORDER BY a.term_desc;
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.first_admit_state_desc, a.version_desc
ORDER BY a.term_desc;
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.first_admit_county_desc, a.version_desc
ORDER BY a.term_desc;
...
SELECT a.term_desc,
  ...
  LEFT JOIN export.academic_programs b
         ON a.primary_program_id = b.program_id
  LEFT JOIN export.term c
         ON a.term_id = c.term_id
      WHERE a.is_enrolled IS TRUE
        AND a.is_primary_level IS TRUE
        AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
   GROUP BY a.term_desc, b.college_desc, b.college_abbrv
   ORDER BY a.term_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, b.department_desc, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, a.primary_major_desc, a.primary_program_desc, a.primary_program_id, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND a.is_degree_seeking IS TRUE
  AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, a.is_degree_seeking, a.version_desc;
```
...
GROUP BY a.term_desc, a.student_type_desc, a.student_type_code, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  ...
  AND a.is_degree_seeking IS TRUE
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, a.level_desc, a.level_id, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  ...
  AND a.is_degree_seeking IS TRUE
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, a.primary_level_class_desc, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, a.full_time_part_time_code, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, a.residency_desc, a.version_desc;
```
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, is_high_school, a.version_desc;

```
## Non-Matriculated
A non-degree-seeking student who is enrolled to persue personal interests, not a credential. A Non-matriculated is a subset of Non-degree-seeking.
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND a.primary_major_campus_id = 'O01'
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, online_program_student, a.version_desc;
...
WHERE a.is_enrolled IS TRUE
  AND a.is_primary_level IS TRUE
  AND DATE_PART('year', NOW()) - b.academic_year_code :: INT <= 5 -- Last 5 Years
GROUP BY a.term_desc, gpa_band, a.version_desc;
...
SELECT a.term_desc,
   ...
   LEFT JOIN export.term c
       ...
       WHERE a.is_enrolled IS TRUE
         AND a.is_primary_level IS TRUE
         AND a.is_pell_eligible IS TRUE
         AND DATE_PART('year', NOW()) - c.academic_year_code :: INT <= 5 -- Last 5 Years
    GROUP BY a.term_desc, a.is_pell_eligible, a.version_desc;

...
## ACT Composite Score
ACT Composite Score is the students highest ACT composite score. The Composite score is the average of the four ACT test scores (Math, English, Reading, Science), rounded to the nearest whole number.

#todo
high_school_code
high_school_desc
high_school_graduation_date
latest_high_school_gpa
marital_status_code
marital_status_desc
religion_code
religion_desc
...
```sql
--This query will pull current data from the current term (use student_version to pull snapshot data)
SELECT a.student_id,
       CASE
...
# Term Based Groups
Data in this classification can change each semester and is based on information from the system for this current term such as program, credit hours attempted, etc. This data is stored at the student term level, in the student_term_level table. This data changes often. This data can be pulled as of Current, End of Term or Census.
...
## Course Load
Course load is the status of a student's time commitment based off of how many credit hours attempted and the student's level for a given semester. The options are: 
...
A Non-Matriculated student is a non-degree-seeking student who is enrolled to pursue personal interests, not a credential. A Non-matriculated is a subset of Non-degree-seeking. If the sql output is TRUE then the student is a non-matriculated student. 
...
Institutional Cumulative GPA Band displays a student's Institutional Cumulative (across all semesters) GPA. Best used at End of Term. Reported as a grouping: 
...
- 2.5-2.999
- 3-4.0
- missing (does not have an end of term record or is in a year long High School course)
...
SELECT a.student_id,
       ...
       END AS overall_cumulative_gpa_band,
        CASE
           ...
           ELSE 'new'
        END AS overall_institutional_gpa_band,
       a.is_pell_eligible,
       a.is_pell_awarded,
       a.is_pell_awarded,
...
# Course Based Groups
Data in this classification can change each semester and is based on courses the student is enrolled in. This data is stored at the student term section level, in the student_term_section table. This data is changes often. This data can be pulled as of Current, End of Term or Census.

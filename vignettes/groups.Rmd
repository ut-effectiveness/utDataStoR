...
## Age Band
How old a student is today based on their birth date. This is typically reported in an age band: <18; 18-24; 25-34; 35-44; 44-59; >= 60.
...
A students who is eligible to receive financial aid and earn a degree. Data based off of the program the student has declared. If TRUE the student is degree seeking. If FALSE then students are non-degree seeking.
...
## Class Level
A student's hierarchical status which is classified on the basis of earned credit hours for undergraduate students. The options are: Freshman <30 credit hours; Sophomore 30-59 credit hours; Junior 60-89 credit hours; Senior 90 or more credit hours. All graduate students are reported as Graduate.
...
## Residency
A students tuition rate. The options are: Resident all those paying resident tuition; Non-Resident all those paying non-resident tuition; Good Neighbor paying good neighbor tuition.
...
A High School student enrolled in at least one funded concurrent enrollment class. If TRUE then the student is a High School Concurrent Enrollment Student. If FALSE then the studen is a High School Student Other.

## SQL for High School Concurrent Enrolment Student
```sql
WITH cte AS
           (
...
## SQL for Distance Education Student and Online Student

```sql
SELECT a.student_id,
       a.course_id,
       CASE
           WHEN b.is_distance_ed_some OR b.is_distance_ed_all IS TRUE THEN 'TRUE'
           ELSE FALSE
        END AS is_distance_ed,
       CASE
           WHEN a.instruction_method_desc = 'I' THEN 'TRUE'
           ELSE FALSE
        END AS is_online
  FROM export.student_section a
LEFT JOIN export.student_term_level b ON b.student_id = a.student_id AND b.term_id = a.term_id AND b.is_primary_level IS TRUE
 WHERE a.is_enrolled IS TRUE
   AND a.is_current_term IS TRUE
   ```

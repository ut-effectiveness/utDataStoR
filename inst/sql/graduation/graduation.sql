...
This query provides a single row for every credential awarded, if a student earns more than one credential then the
   student will be listed more than once for the current graduated academic year plus 5 years.
The degrees awarded table is the base table. It is joined to the academic programs table to get the correct college,
   and department data, along with the major, cip code, IPEDS award level and the graduated degree type. It is joined
   to the term table to get the season. Finally, it is joined to the student table to get the current student
   demographic data. */
